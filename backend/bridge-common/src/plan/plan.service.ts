...
export class PlanService {
  ...
  getAllPlan(): Promise<PlnEntity[]> {
    return this.plnRepository.find({where: {
      delYn: 'N'
    }});
  }

  getPlanById(planId: string): Promise<PlnEntity> {
    return this.plnRepository.findOne({where: {
      id: planId,
      delYn: 'N'
    }});
  }

  getPlannedPlan(mbrId: string): Promise<PlnEntity[]> {
    return this.plnRepository.find({where: {
      createMbrId: mbrId,
      delYn: 'N'
